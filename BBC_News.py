...
#保存如数据库
#取消缩略图保存
#datetime 标签可以直接提取到时间 √ 注意 保存的是当地时间
#Video 标签 处理视频类
import urllib
import urllib2
import re
...
class BBC:
    ...
    def getPostData(self):
        ...
        count = 1
        for item in result:
            pattern2 = re.compile('datetime="(.*?)Z" data-seconds="',re.S)
            result2 = re.findall(pattern2,item)
            timeitem = str(result2[0]).replace("T"," ").replace(".000","")
    ...
    def saveAllToDb(self, contentsfile , contentsimg):
        for (contentfile ,contentimg) in zip(contentsfile , contentsimg):
            ...
            cur.execute("insert ignore into news(news_ID,news_URL,news_Title,news_Data,news_From,news_MiniImg,news_Top_n,news_InTime) values('%d','%s','%s','%s','%s','%s','%s','%s') "   % (0,str(dbTemp[1]),str(dbTemp[2]),str(dbTemp[5]),"BBC",str(contentimg).split('\n')[1],str(dbTemp[0]),time.strftime( ISOTIMEFORMAT, time.localtime() )))
            conn.commit()
